...
class DDPM(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("global_step", self.global_step, prog_bar=True, logger=True, on_step=True, on_epoch=False, sync_dist=True)
...
class LatentDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, c):
        if self.cond_stage_forward is None:
            if self.cond_stage_model == "__is_first_stage__":
                c = self.get_first_stage_encoding(self.encode_first_stage(c))
    ...
    def progressive_denoising(
        ...
        if cond is not None:
            if isinstance(cond, dict):
                cond = {
                    key: cond[key][:batch_size] if not isinstance(cond[key], list) else list(map(lambda x: x[:batch_size], cond[key]))
                    for key in cond
                }
            else:
